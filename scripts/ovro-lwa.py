...
from pygsk import plot as pygsk_plot
...
def _normalize_scale_arg(vals: List[str]) -> str | List[str]:
    """
    Clean up and validate the --scale values.
    Accepts a list of strings (nargs='+'), returns either a single string
    ('linear'/'log') or a list of such strings (length 1–4).
    """
    cleaned: List[str] = []
    for v in vals:
        # Allow list-like input such as: [log, log, log, linear]
        v2 = v.strip().strip("[],")
        if not v2:
            continue
        if v2 not in ("linear", "log"):
            raise ValueError(f"Invalid scale '{v2}'; expected 'linear' or 'log'.")
        cleaned.append(v2)
    ...
    if len(cleaned) == 1:
        return cleaned[0]
    if len(cleaned) > 4:
        raise ValueError("At most 4 scale values allowed for [raw, SK, S1, flags].")
    return cleaned


def _normalize_float_seq(vals: Optional[List[float]]) -> Optional[float | List[float]]:
    """
    Turn None → None, single-element list → scalar, multi-element → list.
    """
    if vals is None:
        return None
    if len(vals) == 1:
...
def main() -> None:
    ...
    # Control whether figures are shown interactively
    ap.add_argument(
        "--no-show",
        action="store_true",
        help="Do not display plots; only save them if --save-plot is set.",
    )
    ...
    # Context image scaling (support per-panel control)
    # Panels are [raw, SK, S1, flags]; flags is categorical and ignores scale/vmin/vmax,
    # but we accept a 4th value for consistency.
    ap.add_argument(
        "--scale",
        nargs="+",
        default=["linear"],
        help=(
            "Scaling for context image. Either a single value ('linear' or 'log') "
            "or up to 4 values for [raw, SK, S1, flags], e.g.: "
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Minimum for context image scaling. Either a single float or up to 4 "
            "floats for [raw, SK, S1, flags]."
        ),
    )
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Maximum for context image scaling. Either a single float or up to 4 "
            "floats for [raw, SK, S1, flags]."
        ),
    )
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Floor value(s) for log scaling. Either a single float or up to 4 "
            "floats for [raw, SK, S1, flags]."
        ),
    )
    ...
    # Normalize per-panel arguments
    scale_arg   = _normalize_scale_arg(args.scale)
    vmin_arg    = _normalize_float_seq(args.vmin)
    vmax_arg    = _normalize_float_seq(args.vmax)
    log_eps_arg = _normalize_float_seq(args.log_eps)
    ...
    # Compute SK only (no internal plotting)
    res1 = run_sk_test(
        precomputed=pre1,
        pfa=args.pfa,
        plot=False,
    ...
    # External plotting control: only if showing or saving
    if show_plots or args.save_plot:
        pygsk_plot.plot_sk_histogram(
            res1,
            log_x=args.log_x,
            log_bins=args.log_bins,
            log_count=args.log_count,
